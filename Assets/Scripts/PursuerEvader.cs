...
public class PursuerEvader : MonoBehaviour {
	...
	void Update () {
        ...
        else if (Input.GetMouseButton(1))
        {
            ...
            if(mouseVelocity != 0)
            {
                estimatedPos = mousepos * mouseVelocity * Time.deltaTime;
            }
            Vector3 direction = estimatedPos - transform.position;
            direction.z = 0;
            float myMoveSpeed = linearVelocity.magnitude;
            if (myMoveSpeed > maxSpeed)
            {
                myMoveSpeed = maxSpeed;
            }
            transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction), angularVelocity.magnitude * Time.deltaTime);
            transform.position = transform.position - direction.normalized * myMoveSpeed * Time.deltaTime;
        }
	}
}
